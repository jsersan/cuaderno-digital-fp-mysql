...
/**
 * ============================================================================
 *  simular-asistencia-apin.mjs   (curso 2026-2027)
 *
 *  Simula un curso de ASISTENCIA (sept 2026 → feb 2027) para APIN / SM2.
 ...
 *  simular-notas-apin.mjs: si usas la misma --seed, los alumnos que más faltan
 *  son los que peor van de notas (coherencia notas ↔ asistencia).
 *
 *  NO crea alumnos: usa los matriculados en APIN (o, si no hay matrícula
 *  explícita, todos los del grupo).
 *
 *  USO:
 *    node scripts/simular-asistencia-apin.mjs            (dry-run)
 *    node scripts/simular-asistencia-apin.mjs --commit   (escribe)
 *    node scripts/simular-asistencia-apin.mjs --commit --limpiar
 *    node scripts/simular-asistencia-apin.mjs --seed=2026 --grupo=ID --curso=2026-2027
 * ============================================================================
 */
...
// Franjas en las que APIN se imparte (genera un registro de "pasar lista" por franja y día)
const FRANJAS_APIN = ['08:30-09:25', '09:25-10:20'];
...
// PRNG determinista (mulberry32) — idéntico al de simular-notas-apin.mjs
function makeRng(seed) {
  let a = seed >>> 0;
  return function () { a |= 0; a = (a + 0x6D2B79F5) | 0; let t = Math.imul(a ^ (a >>> 15), 1 | a); t = (t + Math.imul(t ^ (t >>> 7), 61 | t)) ^ t; return ((t ^ (t >>> 14)) >>> 0) / 4294967296; };
}

/** Asigna nivel a cada alumno EXACTAMENTE igual que simular-notas-apin.mjs
...
async function resolverGrupo(db, modulo) {
  ...
  const mismoCiclo = cands.filter(g => g.cicloId === modulo.cicloId);  // APIN vive en su propio ciclo
...
async function main() {
  ...
  // Alumnos matriculados en APIN (fallback: todos los del grupo)
  ...
  for (const { anio, mes } of MESES) {
    ...
    let regsMes = 0;
    for (const dia of dias) {
      for (const franja of FRANJAS_APIN) {
