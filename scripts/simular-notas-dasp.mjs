#!/usr/bin/env node
/**
 * simular-notas-dasp.mjs
 * ==========================================================================
 * Simula TODAS las notas de DASP / SM2 (curso 2026-2027) con una distribución
 ...
 * (coherente con simular-asistencia-dasp.mjs si usas la MISMA --seed).
 *
 ...
 *
 * USO:
 *   node scripts/simular-notas-dasp.mjs                 # DRY-RUN (no escribe)
 *   node scripts/simular-notas-dasp.mjs --commit        # escribe
 *   node scripts/simular-notas-dasp.mjs --commit --limpiar   # borra notas previas antes
 *   node scripts/simular-notas-dasp.mjs --seed=42
 *   node scripts/simular-notas-dasp.mjs --grupo=ID --curso=2026-2027
 * ==========================================================================
 */
...
async function localizar() {
  ...
  // Grupo: por --grupo=ID, o por nombre+curso, o el que más matriculados en DASP tenga.
  ...
  if (!cands.length) cands = grupos.filter(g => norm(g.nombre).includes(norm(NOMBRE_GRUPO)) && (!CURSO || g.cursoAcademico === CURSO));
    // Prioriza los grupos del MISMO ciclo que el módulo (DASP/SM2 lee del grupo activo del ciclo)
...
async function run() {
  console.log(`\n${'='.repeat(64)}`);
  console.log(`  SIMULAR NOTAS DASP / ${NOMBRE_GRUPO}  ·  curso ${CURSO}  ·  semilla ${SEED}`);
  ...
  // Alumnos del grupo matriculados en DASP (fallback: todos los del grupo)
  ...
  for (const ev of evalsExamen) {
    ...
    recuperaciones.push({
      titulo: `Recuperación ${ev}: DASP`,
      descripcion: `Recuperación de la ${ev} del módulo Digitalización aplicada a los sectores productivos.`,
  ...
  if (LIMPIAR) {
    console.log('\n🧹 Limpiando recuperaciones y calificaciones previas de DASP/' + grupo.nombre + '...');
  ...
  console.log('   simular-asistencia-dasp.mjs con la MISMA --seed.\n');
  process.exit(0);
}
