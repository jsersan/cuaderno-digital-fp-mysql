#!/usr/bin/env node
/**
 * simular-notas-dbdr.mjs
 * ==========================================================================
 * Simula TODAS las notas de DBDR / SM2 (curso 2026-2027) con una distribución
 ...
 * (coherente con simular-asistencia-dbdr.mjs si usas la MISMA --seed).
 *
 ...
 *
 * USO:
 *   node scripts/simular-notas-dbdr.mjs                 # DRY-RUN (no escribe)
 *   node scripts/simular-notas-dbdr.mjs --commit        # escribe
 *   node scripts/simular-notas-dbdr.mjs --commit --limpiar   # borra notas previas antes
 *   node scripts/simular-notas-dbdr.mjs --seed=42
 *   node scripts/simular-notas-dbdr.mjs --grupo=ID --curso=2026-2027
 * ==========================================================================
 */
...
const ABREV_MODULO = 'DBDR';
const NOMBRE_GRUPO = '2SM2';
...
async function localizar() {
  ...
  // Grupo: por --grupo=ID, o por nombre+curso, o el que más matriculados en DBDR tenga.
  ...
  if (!cands.length) cands = grupos.filter(g => norm(g.nombre).includes(norm(NOMBRE_GRUPO)) && (!CURSO || g.cursoAcademico === CURSO));
    // Prioriza los grupos del MISMO ciclo que el módulo (DBDR/SM2 lee del grupo activo del ciclo)
...
async function run() {
  console.log(`\n${'='.repeat(64)}`);
  console.log(`  SIMULAR NOTAS DBDR / ${NOMBRE_GRUPO}  ·  curso ${CURSO}  ·  semilla ${SEED}`);
  ...
  // Alumnos del grupo matriculados en DBDR (fallback: todos los del grupo)
  ...
  else console.log('⚠ Ningún alumno tiene matrícula explícita en DBDR; uso todos los del grupo.');
  ...
  for (const ev of evalsExamen) {
    ...
    recuperaciones.push({
      titulo: `Recuperación ${ev}: DBDR`,
      descripcion: `Recuperación de la ${ev} del módulo Diseño de Bases de Datos Relacionales.`,
  ...
  if (LIMPIAR) {
    console.log('\n🧹 Limpiando recuperaciones y calificaciones previas de DBDR/' + grupo.nombre + '...');
  ...
  console.log('   simular-asistencia-dbdr.mjs con la MISMA --seed.\n');
  process.exit(0);
}
