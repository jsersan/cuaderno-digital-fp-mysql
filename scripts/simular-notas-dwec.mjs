#!/usr/bin/env node
/**
 * simular-notas-dwec.mjs
 * ==========================================================================
 * Simula TODAS las notas de DWEC / 2AW3 (curso 2026-2027) con una distribución
 ...
 * (coherente con simular-asistencia-dwec.mjs si usas la MISMA --seed).
 *
 ...
 *
 * USO:
 *   node scripts/simular-notas-dwec.mjs                 # DRY-RUN (no escribe)
 *   node scripts/simular-notas-dwec.mjs --commit        # escribe
 *   node scripts/simular-notas-dwec.mjs --commit --limpiar   # borra notas previas antes
 *   node scripts/simular-notas-dwec.mjs --seed=42
 *   node scripts/simular-notas-dwec.mjs --grupo=ID --curso=2026-2027
 * ==========================================================================
 */
...
const ABREV_MODULO = 'DWEC';
const NOMBRE_GRUPO = '2AW3';
...
/** Asigna nivel a cada alumno de forma determinista: 25% flojo (suspenden), resto repartido. */
...
async function localizar() {
  ...
  // Grupo: por --grupo=ID, o por nombre+curso, o el que más matriculados en DWEC tenga.
  ...
  if (!cands.length) cands = grupos.filter(g => norm(g.nombre).includes(norm(NOMBRE_GRUPO)) && (!CURSO || g.cursoAcademico === CURSO));
    if (cands.length === 1) grupo = cands[0];
      ...
      // desempatar por nº de alumnos matriculados en DWEC
...
async function run() {
  console.log(`\n${'='.repeat(64)}`);
  console.log(`  SIMULAR NOTAS DWEC / ${NOMBRE_GRUPO}  ·  curso ${CURSO}  ·  semilla ${SEED}`);
  ...
  // Alumnos del grupo matriculados en DWEC (fallback: todos los del grupo)
  ...
  else console.log('⚠ Ningún alumno tiene matrícula explícita en DWEC; uso todos los del grupo.');
  ...
  for (const ev of TODAS_EVALS) {
    ...
    for (const a of alumnos) {
      ...
      const calcFinal = () => round1(clamp(W.ex*nEx + W.ta*nTa + W.ac*actitud + W.as*asist, 0, 10));
      let final = calcFinal();
      ...
      calificacionesFinales.push({
        ...
        evaluacion: ev,
        notaExamenes: nEx, notaTareas: nTa, notaActitud: round1(actitud), notaAsistencia: asist,
  ...
  for (const ev of evalsExamen) {
    ...
    recuperaciones.push({
      titulo: `Recuperación ${ev}: DWEC`,
      descripcion: `Recuperación de la ${ev} del módulo Desarrollo Web en Entorno Cliente.`,
  ...
  if (LIMPIAR) {
    console.log('\n🧹 Limpiando recuperaciones y calificaciones previas de DWEC/' + grupo.nombre + '...');
  ...
  console.log('   simular-asistencia-dwec.mjs con la MISMA --seed.\n');
  process.exit(0);
}
