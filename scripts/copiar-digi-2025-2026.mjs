...
async function run() {
  console.log(`\n${'═'.repeat(64)}`);
  console.log(`  COPIAR DIGI · ${CURSO_ORIGEN} → ${CURSO_DESTINO} (fechas ${ANIOS} año)`);
  ...
  (await db.collection('grupos').get()).forEach(d => {
    const g = d.data();
    if (norm(g.nombre) === 'sm2') daw2[g.cursoAcademico] = { id: d.id, ref: d.ref, ...g };
  });
  const grupoOrigen = daw2[CURSO_ORIGEN];
  const grupoDestino = daw2[CURSO_DESTINO];
  if (!grupoOrigen) { console.error(`❌ No hay grupo SM2 de ${CURSO_ORIGEN} (origen)`); process.exit(1); }
  if (!grupoDestino) { console.error(`❌ No hay grupo SM2 de ${CURSO_DESTINO} (destino)`); process.exit(1); }
  console.log(`✓ SM2 ${CURSO_ORIGEN}: ${grupoOrigen.id} (origen)`);
  console.log(`✓ SM22 ${CURSO_DESTINO}: ${grupoDestino.id} (destino)`);
