...
@Component({
  ...
  template: `
    ...
    <!-- Barra de acciones masivas (solo admin, solo si hay selección) -->
    @if (esAdmin && profesoresSeleccionados.length > 0) {
      <mat-card class="bulk-bar">
        <span class="bulk-count">
          <mat-icon>check_circle</mat-icon> {{ profesoresSeleccionados.length }} seleccionado(s)
        </span>
        <span class="bulk-spacer"></span>
        <button mat-raised-button color="warn" (click)="eliminarProfesoresMasivo()">
          <mat-icon>delete</mat-icon> Eliminar ({{ profesoresSeleccionados.length }})
        </button>
        <button mat-button (click)="limpiarSeleccion()">Cancelar</button>
      </mat-card>
    }
        ...
        <ng-container matColumnDef="select">
          <th mat-header-cell *matHeaderCellDef>
            <mat-checkbox
              ...
              [indeterminate]="visiblesSeleccionados > 0 && !todosVisiblesSeleccionados"
              (change)="toggleTodos($event.checked)">
            </mat-checkbox>
          </th>
          <td mat-cell *matCellDef="let p">
            <mat-checkbox
              [checked]="estaSeleccionado(p.uid)"
              [disabled]="esYoMismo(p)"
              (change)="toggleSeleccion(p.uid)"
        ...
        <ng-container matColumnDef="acciones">
          ...
          <td mat-cell *matCellDef="let p">
            ...
            <mat-menu #rolMenu="matMenu">
              ...
              <!-- Eliminar profesor de la BD: solo administradores -->
              @if (esAdmin) {
                <mat-divider></mat-divider>
  ...
  styles: [`
    ...
    .bulk-bar { display: flex; align-items: center; gap: 12px; margin-bottom: 16px; padding: 8px 16px; border-radius: 12px; border-left: 4px solid #c62828; background: #fff5f5; }
    .bulk-count { display: flex; align-items: center; gap: 6px; font-weight: 600; color: #c62828; }
    .bulk-count mat-icon { font-size: 20px; width: 20px; height: 20px; }
    .bulk-spacer { flex: 1; }
...
export class ProfesoresListComponent implements OnInit {
  ...
  /** Ids (uid) de los profesores marcados con checkbox. */
  seleccion = new Set<string>();

  /** Columnas de la tabla. La columna 'select' solo se muestra al administrador. */
  get cols(): string[] {
    const c = ['nombre', 'email', 'rol', 'modulos', 'estado', 'acciones'];
    return this.esAdmin ? ['select', ...c] : c;
  }
  ...
  /** True si el usuario que ha iniciado sesión es administrador del centro. */
  get esAdmin(): boolean {
    return (this.auth.currentUser?.rol as string) === 'admin';
  }
  ...
  async eliminarProfesor(p: Usuario) {
    if (!p.uid) return;
    if (!this.esAdmin) {
      this.snackBar.open('Solo un administrador puede eliminar profesores.', 'OK', { duration: 4000 });
      return;
    }
    if (this.esYoMismo(p)) {
  ...
  /** Profesores cargados que están marcados (sobre los que actúan las acciones masivas). */
  get profesoresSeleccionados(): Usuario[] {
    return this.profesores.filter(p => p.uid && this.seleccion.has(p.uid));
  }

  /** Profesores visibles que SÍ se pueden seleccionar (excluye al propio usuario). */
  private seleccionablesVisibles(): Usuario[] {
    return this.filteredProfesores.filter(p => !!p.uid && !this.esYoMismo(p));
  }

  /** Nº de profesores visibles seleccionables que están marcados. */
  get visiblesSeleccionados(): number {
    return this.seleccionablesVisibles().filter(p => this.seleccion.has(p.uid!)).length;
  }

  /** True si TODO lo visible/filtrado seleccionable está marcado. */
  get todosVisiblesSeleccionados(): boolean {
    const sel = this.seleccionablesVisibles();
    return sel.length > 0 && this.visiblesSeleccionados === sel.length;
  }

  estaSeleccionado(uid?: string): boolean {
    return !!uid && this.seleccion.has(uid);
  }

  toggleSeleccion(uid?: string) {
    if (!uid) return;
    if (this.seleccion.has(uid)) this.seleccion.delete(uid);
    else this.seleccion.add(uid);
  }

  /** El checkbox de cabecera marca/desmarca solo lo visible seleccionable (sin incluirte a ti). */
  toggleTodos(checked: boolean) {
    for (const p of this.seleccionablesVisibles()) {
      if (checked) this.seleccion.add(p.uid!);
      else this.seleccion.delete(p.uid!);
    }
  }
  ...
  /** Borrado masivo REAL de profesores (Firestore). Excluye al propio usuario. */
  async eliminarProfesoresMasivo() {
    if (!this.esAdmin) return;
    const items = this.profesoresSeleccionados.filter(p => p.uid && !this.esYoMismo(p));
    if (items.length === 0) {
      this.snackBar.open('No hay profesores seleccionados que puedas eliminar.', 'OK', { duration: 4000 });
      return;
    }

    const ok = await this.confirm.ask({
      title: `Eliminar ${items.length} profesor(es)`,
      message:
        `Vas a eliminar ${items.length} profesor(es) de la base de datos. Se borra su ficha de ` +
        `Firestore (su cuenta de acceso en Auth no se elimina desde aquí).\n\n` +
    ...
    const results = await Promise.allSettled(items.map(async p => {
      // Si es tutor del grupo activo, liberar la tutoría antes de borrar.
      if (grupo?.id && grupo.tutorId && p.uid === grupo.tutorId) {
        await updateDoc(doc(this.firestore, `grupos/${grupo.id}`), { tutorId: '', updatedAt: new Date() });
        this.activeModule.setGrupo({ ...grupo, tutorId: '' });
      }
      await deleteDoc(doc(this.firestore, `usuarios/${p.uid}`));
    }));

    const borradosUids = items.filter((_, i) => results[i].status === 'fulfilled').map(p => p.uid);
    const errores = results.filter(r => r.status === 'rejected').length;

    // collectionData refresca en tiempo real; quitamos también de la lista local por inmediatez.
    this.profesores = this.profesores.filter(x => !borradosUids.includes(x.uid));
    this.limpiarSeleccion();
    this.filter();
    this.snackBar.open(
      `✓ ${borradosUids.length} eliminado(s)` + (errores ? ` · ${errores} con error` : ''),
      'OK', { duration: 5000 }
    );
  }
